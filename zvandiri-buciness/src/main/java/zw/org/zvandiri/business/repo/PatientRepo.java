...
public interface PatientRepo extends AbstractRepo<Patient, String> {
    ...
    @Query("Select Distinct p from Patient p left join fetch p.createdBy left join fetch p.modifiedBy left join fetch p.education left join fetch p.educationLevel left join fetch p.referer left join fetch p.primaryClinic " +
            "left join fetch p.supportGroup where p.primaryClinic is not null  and" +
            //"p.active=:active and " +
            "(p.firstName Like :name% or p.lastName Like :name% or p.patientNumber Like :name%) order by p.lastName, p.firstName, p.middleName ASC")
    public List<Patient> findByFirstNameOrLastName(@Param("name") String name);
    
    @Query("Select Distinct p from Patient p left join fetch p.createdBy left join fetch p.modifiedBy  left join fetch p.education " +
            "left join fetch p.educationLevel left join fetch p.referer left join fetch p.primaryClinic left join fetch p.supportGroup where p.primaryClinic is not null  and " +
            //"p.active=:active and " +
            "(p.firstName Like :name% or p.lastName Like :name% or p.patientNumber Like :name%) and p.primaryClinic.district.province=:province order by p.lastName, p.firstName, p.middleName ASC")
    public List<Patient> findByFirstNameOrLastNameAndProvince(@Param("name") String name,  @Param("province") Province province);
    
    @Query("Select Distinct p from Patient p left join fetch p.createdBy left join fetch p.modifiedBy left join fetch p.education " +
            "left join fetch p.educationLevel left join fetch p.referer left join fetch p.primaryClinic left join fetch p.supportGroup where p.primaryClinic is not null  and " +
            //"p.active=:active and " +
            "(p.firstName Like :name% or p.lastName Like :name% or p.patientNumber Like :name%) and p.primaryClinic.district=:district order by p.lastName, p.firstName, p.middleName ASC")
    public List<Patient> findByFirstNameOrLastNameAndDistrict(@Param("name") String name, @Param("district") District district);
    
    @Query("Select Distinct p from Patient p left join fetch p.createdBy left join fetch p.modifiedBy left join fetch p.education " +
            "left join fetch p.educationLevel left join fetch p.referer left join fetch p.primaryClinic left join fetch p.supportGroup  where p.primaryClinic is not null  and " +
            //"p.active=:active and " +
            "((p.firstName Like :first% or p.firstName Like :last%  or p.patientNumber Like :last% or p.patientNumber Like :first%) and (p.lastName Like :last% or p.lastName Like :first% or p.patientNumber Like :first% or p.patientNumber Like :last%) ) order by p.lastName, p.firstName, p.middleName ASC")
    public List<Patient> findByFirstNameAndLastName(@Param("first") String first, @Param("last") String last);
    
    @Query("Select Distinct p from Patient p left join fetch p.createdBy left join fetch p.modifiedBy left join fetch p.education " +
            "left join fetch p.educationLevel left join fetch p.referer left join fetch p.primaryClinic left join fetch p.supportGroup  where p.primaryClinic is not null  and " +
            //"p.active=:active and " +
            "((p.firstName Like :first% or p.firstName Like :last%) and (p.lastName Like :last% or p.lastName Like :first%) ) and p.primaryClinic.district.province=:province order by p.lastName, p.firstName, p.middleName ASC")
    public List<Patient> findByFirstNameAndLastNameAndProvince(@Param("first") String first, @Param("last") String last,  @Param("province") Province province);
    
    @Query("Select Distinct p from Patient p left join fetch p.createdBy left join fetch p.modifiedBy left join fetch p.education " +
            "left join fetch p.educationLevel left join fetch p.referer left join fetch p.primaryClinic left join fetch p.supportGroup  where p.primaryClinic is not null  and " +
            //"p.active=:active and " +
            "((p.firstName Like :first% or p.firstName Like :last%) and (p.lastName Like :last% or p.lastName Like :first%) ) and p.primaryClinic.district=:district order by p.lastName, p.firstName, p.middleName ASC")
    public List<Patient> findByFirstNameAndLastNameAndDistrict(@Param("first") String first, @Param("last") String last,  @Param("district") District district);
    
    @Query("Select Distinct p from Patient p left join fetch p.education left join fetch p.educationLevel left join fetch p.referer left join fetch p.primaryClinic left join fetch p.supportGroup where p.primaryClinic is not null  and p.firstName=:firstName and p.lastName=:lastName and p.dateOfBirth=:dateOfBirth and p.gender=:gender")
