...
public interface PatientInnerJoin {
    ...
    public final String PATIENT_FULL_ASSOC_FETCH = "left join fetch p.dependents left join fetch p.medicalHists left join fetch p.chronicInfectionItems left join fetch p.hivConInfectionItems left join fetch p.arvHists left join fetch p.mentalHealthItems left join fetch p.obstercHists left join fetch p.socialHists left join fetch p.substanceItems left join fetch p.investigationTests left join fetch p.referrals left join fetch p.tbIpts left join fetch p.mortalitys left join fetch p.mentalHealthScreenings"/* left join fetch p.period left join fetch p.education left join fetch p.educationLevel left join fetch p.referer left join fetch p.primaryClinic left join fetch p.supportGroup left join fetch p.relationship left join fetch p.mobileOwnerRelation left join fetch p.secondaryMobileownerRelation left join fetch p.motherOfHei left join fetch p.reasonForNotReachingOLevel"*/;
