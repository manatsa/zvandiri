...
public class TBScreeningCandidatesTask extends RecursiveTask<List>{
    ...
    public TBScreeningCandidatesTask(List<String> patientIDS, PatientReportService reportService, SearchDTO searchData) {
    ...
    protected List compute() {
        if (patientIDS.size() <= ReportGenConstants.SEQUENTIAL_THRESHOLD) {
            ...
            TBScreeningCandidatesTask task = new TBScreeningCandidatesTask(patientIDS.subList(0, mid), reportService, searchData);
            TBScreeningCandidatesTask last = new TBScreeningCandidatesTask(patientIDS.subList(mid, patientIDS.size()), reportService, searchData);
            task.fork();
...
//        searchData.setPageSize(pageSize);
        return reportService.getTBScreeningCandidates(patientIDS,searchData);
    }
}
