 ...
 * @author tasu
 */
public class LabResultTask extends RecursiveTask<List>{
    
    private final PatientReportService reportService;
    private final SearchDTO searchData;
    private final List<Integer> arrCount;

    public LabResultTask(List<Integer> arrCount, PatientReportService reportService, SearchDTO searchData) {
    ...
    protected List compute() {
        if (arrCount.size() <= ReportGenConstants.SEQUENTIAL_THRESHOLD) {
            ...
            LabResultTask task = new LabResultTask(arrCount.subList(0, mid), reportService, searchData);
            LabResultTask last = new LabResultTask(arrCount.subList(mid, arrCount.size()), reportService, searchData);
            task.fork();
    ...
    private List process() {
        ...
        return reportService.getPatientLabResultsList(searchData.getInstance(searchData));
    }
}
