...
public class UniqueContactTask extends RecursiveTask<List>{
    ...
    public UniqueContactTask(List<Integer> arrCount, ContactReportService reportService, SearchDTO searchData) {
    ...
    protected List compute() {
        if (arrCount.size() <= ReportGenConstants.SEQUENTIAL_THRESHOLD) {
            ...
            UniqueContactTask task = new UniqueContactTask(arrCount.subList(0, mid), reportService, searchData);
            UniqueContactTask last = new UniqueContactTask(arrCount.subList(mid, arrCount.size()), reportService, searchData);
            task.fork();
    ...
    private List process() {
        ...
        return reportService.getUnique(searchData.getInstance(searchData));
    }
}
