...
public class LocAgeGroupController extends BaseController {
    ...
    @Resource
    private LocAgeGroupReportService locAgeGroupReportService;

    public void setUpModel(ModelMap model, SearchDTO item) {
        item = getUserLevelObjectState(item);
        model.addAttribute("pageTitle", APP_PREFIX + "Location Age Group Cross Tabulation Report");
        ...
        model.addAttribute("excelExport", "/report/aggregate/loc-age-group-healthcenter/export/excel" + item.getQueryString(item.getInstance(item)));
        model.addAttribute("items", locAgeGroupReportService.getDefaultReport(item.getInstance(item)));
    }

    @RequestMapping(value = "/loc-age-group-healthcenter", method = RequestMethod.GET)
    public String getReportIndex(ModelMap model) {
        setUpModel(model, new SearchDTO());
        return "report/table";
    }

    @RequestMapping(value = "/loc-age-group-healthcenter", method = RequestMethod.POST)
    ...
    public void setUpModelB(ModelMap model, SearchDTO item) {
        ...
        model.addAttribute("item", item.getInstance(item));
        model.addAttribute("excelExport", "/report/aggregate/loc-age-group-support-group/export/excel" + item.getQueryString(item.getInstance(item)));
        model.addAttribute("items", locAgeGroupReportService.getDefaultReportB(item.getInstance(item)));
    }

    @RequestMapping(value = "/loc-age-group-support-group", method = RequestMethod.GET)
    public String getReportSupportGroup(ModelMap model) {
        setUpModelB(model, new SearchDTO());
        return "report/table";
    }

    @RequestMapping(value = "/loc-age-group-support-group", method = RequestMethod.POST)
