 ...
 */
@Component
public class ChronicInfectionValidator implements Validator {
    ...
    public boolean supports(Class<?> type) {
        return type.equals(ChronicInfection.class);
    }
    ...
    public void validate(Object o, Errors errors) {
         ...
         */
        ChronicInfection current = (ChronicInfection) o;
        ChronicInfection old = null;
        if (current.getId() != null) {
            old = chronicInfectionService.get(current.getId());
        }
        if (chronicInfectionService.checkDuplicate(current, old)) {
