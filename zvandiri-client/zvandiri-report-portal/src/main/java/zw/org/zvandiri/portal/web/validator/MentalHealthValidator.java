 ...
 */
@Component
public class MentalHealthValidator implements Validator {
    ...
    public boolean supports(Class<?> type) {
        return type.equals(MentalHealth.class);
    }
    ...
    public void validate(Object o, Errors errors) {
         ...
         */
        MentalHealth current = (MentalHealth) o;
        MentalHealth old = null;
        if (current.getId() != null) {
            old = mentalHealthService.get(current.getId());
        }
        if (mentalHealthService.checkDuplicate(current, old)) {
