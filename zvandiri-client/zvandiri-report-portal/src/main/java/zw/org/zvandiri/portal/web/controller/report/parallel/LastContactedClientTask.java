...
public class LastContactedClientTask extends RecursiveTask<List>{
    ...
    public LastContactedClientTask(List<Integer> arrCount, LastContactedService reportService, SearchDTO searchData) {
    ...
    protected List compute() {
        if (arrCount.size() <= ReportGenConstants.SEQUENTIAL_THRESHOLD) {
            ...
            LastContactedClientTask task = new LastContactedClientTask(arrCount.subList(0, mid), reportService, searchData);
            LastContactedClientTask last = new LastContactedClientTask(arrCount.subList(mid, arrCount.size()), reportService, searchData);
            task.fork();
