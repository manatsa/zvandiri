 ...
 */
@Component
public class ActionTakenValidator implements Validator {
    ...
    public boolean supports(Class<?> type) {
        return type.equals(ActionTaken.class);
    }
    ...
    public void validate(Object o, Errors errors) {
         ...
         */
        ActionTaken current = (ActionTaken) o;
        ActionTaken old = null;
        if (current.getId() != null) {
            old = actionTakenService.get(current.getId());
        }
        if (actionTakenService.checkDuplicate(current, old)) {
