 ...
 */
@Component
public class DisabilityCategoryValidator implements Validator {
    ...
    public boolean supports(Class<?> type) {
        return type.equals(DisabilityCategory.class);
    }
    ...
    public void validate(Object o, Errors errors) {
         ...
         */
        DisabilityCategory current = (DisabilityCategory) o;
        DisabilityCategory old = null;
        if (current.getId() != null) {
            old = disabilityCategoryService.get(current.getId());
        }
        if (disabilityCategoryService.checkDuplicate(current, old)) {
