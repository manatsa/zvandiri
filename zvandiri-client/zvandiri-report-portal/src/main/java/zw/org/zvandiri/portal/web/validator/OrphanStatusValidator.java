 ...
 */
@Component
public class OrphanStatusValidator implements Validator {
    ...
    public boolean supports(Class<?> type) {
        return type.equals(OrphanStatus.class);
    }
    ...
    public void validate(Object o, Errors errors) {
         ...
         */
        OrphanStatus current = (OrphanStatus) o;
        OrphanStatus old = null;
        if (current.getId() != null) {
            old = orphanStatusService.get(current.getId());
        }
        if (orphanStatusService.checkDuplicate(current, old)) {
