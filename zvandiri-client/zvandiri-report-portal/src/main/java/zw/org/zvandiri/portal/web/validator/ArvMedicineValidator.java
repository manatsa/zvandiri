 ...
 */
@Component
public class ArvMedicineValidator implements Validator {
    ...
    public boolean supports(Class<?> type) {
        return type.equals(ArvMedicine.class);
    }
    ...
    public void validate(Object o, Errors errors) {
         ...
         */
        ArvMedicine current = (ArvMedicine) o;
        ArvMedicine old = null;
        if (current.getId() != null) {
            old = arvMedicineService.get(current.getId());
        }
        if (arvMedicineService.checkDuplicate(current, old)) {
