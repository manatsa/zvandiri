 ...
 * @author tasu
 */
public class MentalHealthTask extends RecursiveTask<List>{
    
    private final MentalHealthScreeningService reportService;
    private static final int SEQUENTIAL_THRESHOLD = 500;
    private final SearchDTO searchData;
    private final List<Integer> arrCount;

    public MentalHealthTask(List<Integer> arrCount, MentalHealthScreeningService reportService, SearchDTO searchData) {
    ...
    protected List compute() {
        if (arrCount.size() <= SEQUENTIAL_THRESHOLD) {
            ...
            MentalHealthTask task = new MentalHealthTask(arrCount.subList(0, mid), reportService, searchData);
            MentalHealthTask last = new MentalHealthTask(arrCount.subList(mid, arrCount.size()), reportService, searchData);
            task.fork();
