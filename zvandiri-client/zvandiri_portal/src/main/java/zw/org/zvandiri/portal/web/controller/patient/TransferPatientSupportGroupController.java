 ...
 */
@Controller
@RequestMapping("/patient/change-support-group")
public class TransferPatientSupportGroupController extends BaseController {
    ...
    public String setUpModel(ModelMap model, PatientDTO item) {
        ...
        if (item.getPatient().getProvince() != null) {
            ...
            if (item.getPatient().getDistrict() != null) {
                model.addAttribute("supportGroups", supportGroupService.getByDistrict(item.getPatient().getDistrict()));
            }
        }
        return "patient/patientChangeSupportGroupForm";
    }
    ...
    public String saveItem(ModelMap model, @ModelAttribute("item") PatientDTO item, BindingResult result) {
        ...
        patientChangeEventValidator.validateChangeSupportGroup(item, result);
        if (result.hasErrors()) {
