...
public class RelationshipValidator implements Validator {
    ...
    public boolean supports(Class<?> type) {
        return type.equals(Relationship.class);
    }
    ...
    public void validate(Object o, Errors errors) {
         ...
         */
        Relationship current = (Relationship) o;
        Relationship old = null;
        if (current.getId() != null) {
            old = relationshipService.get(current.getId());
        }
        if (relationshipService.checkDuplicate(current, old)) {
