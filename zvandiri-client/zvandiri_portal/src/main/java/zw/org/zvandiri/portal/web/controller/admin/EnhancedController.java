...
public class EnhancedController extends BaseController {
    ...
    public String saveItem(@ModelAttribute("item") @Valid Enhanced item, BindingResult result, ModelMap model) {
        enhancedValidator.validate(item, result);
    ...
    public String itemList(ModelMap model, @RequestParam(required = false) Integer type) {
        model.addAttribute("message", new AppMessage.MessageBuilder().build());
        model.addAttribute("pageTitle", APP_PREFIX + "Enhanced Followup List");
        model.addAttribute("items", enhancedService.getAll());
        if (type != null) {
    ...
    public String getDeleteForm(@RequestParam("id") String id, ModelMap model) {
        Enhanced item = enhancedService.get(id);
        ItemDeleteDTO dto = new ItemDeleteDTO(id, item.getName() + " Enhanced Followup", "item.list?type=3");
        model.addAttribute("item", dto);
        model.addAttribute("message", new AppMessage.MessageBuilder(Boolean.TRUE).message("Are you sure you want to delete this record").messageType(MessageType.WARNING).build());
        model.addAttribute("pageTitle", APP_PREFIX + "Delete " + item.getName() + " EnhancedFollowup");
        return "admin/deleteItem";
    }

    @RequestMapping(value = "item.delete", method = RequestMethod.POST)
    public String deleteItem(@Valid ItemDeleteDTO dto) {
        enhancedService.delete(enhancedService.get(dto.getId()));
