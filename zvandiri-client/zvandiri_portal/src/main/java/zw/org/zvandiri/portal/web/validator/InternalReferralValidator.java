 ...
 */
@Component
public class InternalReferralValidator implements Validator {
    ...
    public boolean supports(Class<?> type) {
        return type.equals(InternalReferral.class);
    }
    ...
    public void validate(Object o, Errors errors) {
         ...
         */
        InternalReferral current = (InternalReferral) o;
        InternalReferral old = null;
        if (current.getId() != null) {
            old = internalReferralService.get(current.getId());
        }
        if (internalReferralService.checkDuplicate(current, old)) {
