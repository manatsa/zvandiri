 ...
 */
@Component
public class ExternalReferralValidator implements Validator {
    ...
    public boolean supports(Class<?> type) {
        return type.equals(ExternalReferral.class);
    }
    ...
    public void validate(Object o, Errors errors) {
         ...
         */
        ExternalReferral current = (ExternalReferral) o;
        ExternalReferral old = null;
        if (current.getId() != null) {
            old = externalReferralService.get(current.getId());
        }
        if (externalReferralService.checkDuplicate(current, old)) {
